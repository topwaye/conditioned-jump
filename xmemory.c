/*
 * [MEMORY ALLOCATOR OF SQUAREWAVEX]
 *
 * Linux Buddy System has fragments, Squarewavex does NOT have fragments.
 *
 * +-----------+ +-----------+ +-----------+ +-----------+ +-----------+ +-----------+ +-----------+
 * |     |     | |           | |  |  |  |  | |     |     | |           | |     |     | |  |  |  |  |
 * |     |     | |           | |  |  |  |  | |     |     | |           | |     |     | |  |  |  |  |
...
#define ONE_PAGE_AREA_MASK                      0xFFFFF000
#define FIXED_PAGE_AREA_MASK                    0xFFFC0000
#define ONE_PAGE_AREA_SIZE                            4096
#define FIXED_PAGE_AREA_SIZE                        262144
#define MAX_SUBAREA_SIZE                            131072
#define MIN_SUBAREA_SIZE                                32
#define SPA_BIT_FIELD_COUNT                             16  /* 4K / 32+ < 128 / 8 bits = 16 bytes */
#define MPA_BIT_FIELD_COUNT                              8  /* 256K / 4K+ < 64 / 8 bits = 8 bytes */
#define RPA_BIT_FIELD_COUNT                           2048  /* 4G- / 256K < 16384 / 8 bits = 2048 bytes */
#define RPA_DELTA_COUNT                                  2
#define XPA_DIR_START_INDEX                              2  /* top 2 nodes are 0 */
#define SPA_DIR_ARRAY_LEN                               10
#define MPA_DIR_ARRAY_LEN                                9
#define HPA_DIR_ARRAY_LEN                                3
#define XALLOC_NEAR_MEM                                  0
#define XALLOC_URGENT_NEAR_MEM                           1
#define XALLOC_FAR_MEM                                   2
/* any flag we do NOT know yet will be interpreted as XALLOC_NEAR_MEM automatically */
#define XALLOC_MINIMAL_SYSTEM   ( FIXED_PAGE_AREA_SIZE * 4 )
/* 1M = ( 1 data area + 1 mpa_dir_area + 1 spa_dir_area ) + page_context_area */

struct raw_area
{
    void * subarea_set;             /* pages which you can reach */
    void * far_subarea_set;         /* pages which you can NOT reach - access denied */
    int bit_count;                  /* totally actual bits ( set bits + clear bits ) && far_bit0 == bit_count */
    int far_bit_count;              /* totally actual far bits ( set bits + clear bits ) */
    int delta [ RPA_DELTA_COUNT ];  /* delta1 and delta2 offset in bit_count */
    unsigned char bit_field [ RPA_BIT_FIELD_COUNT ];
};

struct sin_page_area
{
    void * dir; /* patch!!! */
    struct sin_page_area * next;
    struct sin_page_area * prior;
    void * subarea_set;
    int bit_count; /* set bits */
    unsigned char bit_field [ SPA_BIT_FIELD_COUNT ]; 
};

struct mul_page_area
{
    void * dir; /* patch!!! */
    struct mul_page_area * next;
    struct mul_page_area * prior;
    void * subarea_set;
    int bit_count; /* set bits */
    unsigned char bit_field [ MPA_BIT_FIELD_COUNT ]; 
};

struct spa_directory
{
    struct sin_page_area chain;
    struct sin_page_area full_chain;
    unsigned long size;
};

struct mpa_directory
{
    struct mul_page_area chain;
    struct mul_page_area full_chain;
    unsigned long size;
};

struct hpa_directory
{
    struct hpa_directory * next;
    struct hpa_directory * prior;
    void * area_set;
    unsigned long size;
};
...
struct page_context
{
    struct sin_page_area * spa;
    struct mul_page_area * mpa;
    struct hpa_directory * hpa_entry;
};

struct mem_allocator
{
    void ( * on_init_raw_mem ) ( void * page_area, unsigned long size, void * far_page_area, unsigned long far_size );
    void ( * on_alloc_raw_mem ) ( void * page_area, unsigned long size );
    void ( * on_free_raw_mem ) ( void * page_area, unsigned long size );
    void ( * on_alloc_one_page_mem ) ( void * page_area, unsigned long size );
    void ( * on_free_one_page_mem ) ( void * page_area, unsigned long size );
    struct page_context * hash_table;    
    struct page_context * far_hash_table;
    struct raw_area raw_memory;
    /* header of spa_directory, NOT in use, except the 1st item */
    struct spa_directory spa_dir_header [ SPA_DIR_ARRAY_LEN ];
    /* header of mpa_directory, NOT in use, except the 1st item */
    struct mpa_directory mpa_dir_header [ MPA_DIR_ARRAY_LEN ];
    /* header of hpa_directory, NOT in use, except the 1st item */
    struct hpa_directory hpa_dir_header [ HPA_DIR_ARRAY_LEN ];
    /* header of spa_directory, NOT in use, except the 1st item */
    struct spa_directory far_spa_dir_header [ SPA_DIR_ARRAY_LEN ];
    /* header of mpa_directory, NOT in use, except the 1st item */
    struct mpa_directory far_mpa_dir_header [ MPA_DIR_ARRAY_LEN ];
    /* header of hpa_directory, NOT in use, except the 1st item */
    struct hpa_directory far_hpa_dir_header [ HPA_DIR_ARRAY_LEN ];
};
 ...
 * spa/dir_area = 7260
 * spa:
 * 1 dir_area = 7260 spas
 * 1 spa = 128 items  ( 4096 / 32 )
 * total items per dir_area = 128 * 7260 = 929,280 items
 * 1 dir data size = 7260 pages = 29,736,960 byte size = 28 M
 *                 ( 7260 / 64 page = 113 mpa * 262144 = 29,622,272 byte size  = 28 M )
 * spa test:
 * 32 M + 32 M + 32 M + 32 M = 128 M = 33,554,432 * 4 = 134,217,728
 * 1 dir + 1 dir + 1 dir + 1 dir
 * mpa:
 * 1 dir_area = 9343 mpas
 * 1 mpa = 64 items  ( 262144 / 4096 )
 * total items per dir_area = 64 * 9343 = 597,952 items
 * 1 dir data size = 9343 fixed_areas = 2,449,211,392 byte size = 2335 M = 2 G
*/
...
int main ( void )
{
    ...
    len1 = 20 * 1024 * 1024;
    len2 = 20 * 1024 * 1024;
    area = malloc ( len1 + len2 );
    if ( ! area )
        return 0;

    xinitialize ( area, len1,  ( unsigned char * )  area + len1, len2 );

    for ( i = 0; i < 10; i ++ )
    {
        /* 131071, 4097, 4095, 100 */
        size = 262145; /* patch!!! this line must be here for reinit */
        p [ i ] = xalloc ( & size, XALLOC_NEAR_MEM );
        printf ( "%d, %u, %u\n", i, size, ( unsigned long ) p [ i ] );
    }
    for ( i = 0; i < 10; i ++ )
    {
        size = xfree2 ( p [ i ] );
    ...
    for ( i = 0; i < 10; i ++ )
    {
        p [ i ] = xalloc_o ( 7, XALLOC_NEAR_MEM );
        printf ( "%d, %u\n", i, ( unsigned long ) p [ i ] );
    }
    for ( i = 0; i < 10; i ++ )
    {
